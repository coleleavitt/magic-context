...
describe.skipIf(!rustPrereqs.ok)("rust classify under the host historian runner", () => {
    ...
    it(
        "still classifies after the module restarts with historian.runner = host",
        async () => {
            const before = (await runTask(false, ["mock-anthropic/mock-sonnet"])) as { ok?: boolean };
            ...
            try {
                ...
                after = { ok: false, error: (error as { message?: string }).message ?? String(error) };
            }
            console.log(`classify under historian.runner=host: ${JSON.stringify(after).slice(0, 600)}`);
