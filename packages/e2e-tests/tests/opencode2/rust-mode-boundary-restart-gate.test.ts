...
function readTrims(logPath: string): number[] {
    return logLines(logPath, "v2 boundary trim: dropped ").map((line) => Number(line.split(" ")[0]));
}
...
function usageForBody(body: Record<string, unknown>): number {
    return Math.round(JSON.stringify(body.input ?? body.messages ?? []).length / 4);
}
    ...
    return {
        ...
        frames: () =>
            readFileSync(trace, "utf8")
                ...
                .split("\n")
                .filter(Boolean)
                .map((line) => JSON.parse(line) as { sessionID: string; at: number; counters: V2StoreReaderDebugCounters }),
    };
}

/** Every path a process holds open, from lsof's machine-readable output. */
function openPaths(pid: number): string[] {
    const out = Bun.spawnSync(["lsof", "-Fn", "-p", String(pid)]).stdout.toString();
    return out
        .split("\n")
...
describe.skipIf(!prereqs.ok)(
    ...
    () => {
        ...
        const spawnOptions = () => ({
            ...
            magicContextConfig: {
                transform_mode: (process.env.GATE_TRANSFORM_MODE ?? "rust") as "rust" | "ts",
        ...
        const modulePid = (): number | null => {
            const file = join(fixture.env.XDG_DATA_HOME!, "cortexkit", "rust-e2e-pids.json");
            if (!existsSync(file)) return null;
            const record = JSON.parse(readFileSync(file, "utf8")) as { pids: Array<{ role: string; pid: number }> };
            return record.pids.find((entry) => entry.role === "module")?.pid ?? null;
        };

        beforeAll(async () => {
            fixture = isolation();
            logPath = join(fixture.env.XDG_DATA_HOME!, "magic-context-boundary-gate.log");
            ...
            lsofTimer = setInterval(() => {
                const pid = modulePid();
                if (!pid) return;
                for (const path of openPaths(pid)) if (/\.db(-wal|-shm)?$/.test(path)) modulePathsSeen.add(path);
            }, 150);
        }, 900_000);
        ...
        it("keeps the boundary and bounded reads across a restart, and single_store off never opens context.db", async () => {
            ...
            await host.stopHost();
            {
                ...
                store
                    .prepare("UPDATE event_sequence SET seq = MAX(seq, ?) WHERE aggregate_id = ?")
            ...
            const boundaries = () =>
                new Set(readCoverage(logPath).map((entry) => entry.markerAt).filter((marker) => marker !== "none"));
            ...
            await driveHistorian({
                ...
                rounds: 24,
                settleMs: 6_000,
                text: (round) => `turn ${round + 1}: durable signal ${round + 1}. ${ballast(3_000)}`,
            });
            ...
            const beforeFrames = observer.frames().filter((frame) => frame.sessionID === session.id);
            console.log(`before restart: boundaries=${[...beforeBoundaries].join(",")} passes=${beforeCoverage.length} trims=${beforeTrims.join(" ")}`);
            console.log(`oc_input before restart: ${beforeCoverage.map((entry) => entry.ocInput).join(" ")}`);

            // single_store off: the module reports "off" and holds no context.db.
            const status = await subc.moduleStatus(session.id, host.cwd, "session.status");
            const singleStore = (status.single_store ?? (status.result as Record<string, unknown> | undefined)?.single_store) as
            ...
            console.log(`first pass after restart: coverage=${JSON.stringify(firstAfter)} trims=${trimsAfterFirst.join(" ")}`);
            ...
            await driveHistorian({
                ...
                satisfied: () => [...boundaries()].some((marker) => !beforeBoundaries.has(marker)),
                rounds: 24,
                settleMs: 6_000,
                text: (round) => `later turn ${round + 1}: durable signal ${round + 1}. ${ballast(3_000)}`,
            });
            await prompt(`final settle turn. ${ballast(500)}`);
            await Bun.sleep(2_000);
            const afterCoverage = readCoverage(logPath).slice(restartedFrom);
            const afterFrames = observer.frames().filter((frame) => frame.sessionID === session.id);
            console.log(`oc_input after restart: ${afterCoverage.map((entry) => `${entry.ocInput}@${entry.markerAt.slice(-8)}`).join(" ")}`);
            ...
            for (const frame of allFrames)
                for (const [name, op] of Object.entries(frame.counters.operations))
                    perOperation[name] = Math.max(perOperation[name] ?? 0, op.maxDecodedRows);
            console.log(`max decoded rows per operation: ${JSON.stringify(perOperation)}`);
            ...
            const perPass = (frames: typeof allFrames) =>
                frames.map((frame, index) => frame.counters.decodedRows - (index === 0 ? 0 : frames[index - 1]!.counters.decodedRows));
            const passCosts = [...perPass(beforeFrames), ...perPass(afterFrames)];
            console.log(`decoded rows per pass: ${passCosts.join(" ")}`);
            console.log(`module .db files seen by lsof: ${[...modulePathsSeen].join(" ")}`);
            ...
            expect([...modulePathsSeen].filter((path) => path.endsWith("context.db") || path.includes("context.db-"))).toEqual([]);
            ...
            console.log(`first pass after restart handed ${firstAfter[0]?.ocInput} of ${conversationalRows} conversational rows to the module`);
            ...
            expect(beforeBoundaries.has(firstAfter[0]?.markerAt ?? "none")).toBe(true);
            expect(firstAfter[0]?.ocInput ?? Number.POSITIVE_INFINITY).toBeLessThan(conversationalRows);
            expect([...boundaries()].some((marker) => !beforeBoundaries.has(marker))).toBe(true);
            for (const frame of allFrames) expect(frame.counters.operations.history).toBeUndefined();
            for (const max of Object.values(perOperation)) expect(max).toBeLessThanOrEqual(100);
            ...
            const steady = [...perPass(beforeFrames).slice(1), ...perPass(afterFrames).slice(1)];
